...
@Component({
  selector: 'app-oi-momentum-config-page',
  standalone: true,
  ...
  template: `
    <section class="page">
      <div class="row">
        <div>
          <h1 class="page-title">OI Momentum Config</h1>
          <p class="page-subtitle">OI Momentum V3 strategy, safety limits, legacy enhancements and per-index session halts. Runtime changes apply immediately.</p>
        </div>
      </div>
          ...
          <div class="v3-panel">
            ...
            @if (legacyEnhOpen) {
              <div class="form-grid">
                ...
                <!-- Charges-aware entry gate v2 -->
                <div class="toggle-row">
                  <span>Charges gate v2 (net-of-charges + resize)
                    <small class="toggle-hint">OFF = legacy filter (skip if premium×qty×3% &lt; ₹500). ON = gate on real round-trip charges and size lots up (within the capital cap) to clear the floor before rejecting — so good low-premium trades aren't dropped.</small></span>
                  <mat-slide-toggle [(ngModel)]="oiConfig.chargesGateV2Enabled" color="primary"></mat-slide-toggle>
                </div>
                <div class="toggle-row">
                  <span>Resize lots up to clear floor
                    <small class="toggle-hint">When ON, increase lots (never above the capital cap) so a strong low-premium signal becomes cost-effective instead of being skipped.</small></span>
                  <mat-slide-toggle [(ngModel)]="oiConfig.chargesGateResizeUp" color="primary"
                    [disabled]="!oiConfig.chargesGateV2Enabled"></mat-slide-toggle>
                </div>
                <mat-form-field appearance="outline">
                  <mat-label>Expected move target (fraction)</mat-label>
                  <input matInput type="number" min="0.01" max="1.0" step="0.01"
                         [(ngModel)]="oiConfig.chargesGateTargetPct" [disabled]="!oiConfig.chargesGateV2Enabled">
                  <mat-hint>0.03 = project gross gain at a 3% move</mat-hint>
                </mat-form-field>
                <mat-form-field appearance="outline">
                  <mat-label>Min net profit after charges (₹)</mat-label>
                  <input matInput type="number" min="0" step="50"
                         [(ngModel)]="oiConfig.chargesGateMinNetProfit" [disabled]="!oiConfig.chargesGateV2Enabled">
                  <mat-hint>Skip the trade unless expected net (gross − real charges) clears this</mat-hint>
                </mat-form-field>
...
export class OiMomentumConfigPageComponent implements OnInit {
