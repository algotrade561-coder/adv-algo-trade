...
public class ConfigSeedLoader {
    ...
    private void seedAppUsersIfEmpty(Map<String, Object> root) {
        ...
        for (Map<String, Object> u : users) {
            ...
            String role = str(u, "role", "USER").toUpperCase();
            // Migrate legacy "SUPER" → "SUPERUSER"
            if ("SUPER".equals(role)) role = "SUPERUSER";
            Object enabledRaw = u.get("enabled");
            boolean enabled = enabledRaw == null || Boolean.parseBoolean(enabledRaw.toString());
            String name = str(u, "name", null);
            ...
            if (!appUserRepository.existsByEmail(normalizedEmail)) {
                ...
                log.info("AppUser seeded: email={} role={} enabled={}", normalizedEmail, role, enabled);
            }
        }
        migrateLegacyRoles();
    }
    ...
    /**
     * One-time migration for rows created by the old single-user app: role "SUPER"
     * (or lowercase variants) → "SUPERUSER". Without this, the session authority
     * becomes ROLE_SUPER and every /admin/** call 403s even though the UI shows
     * the user as a superuser.
     */
    private void migrateLegacyRoles() {
        for (com.algo.trade.auth.AppUser user : appUserRepository.findAll()) {
            String role = user.getRole();
            if (role == null) continue;
            String normalized = role.trim().toUpperCase();
            if ("SUPER".equals(normalized)) normalized = "SUPERUSER";
            if (!normalized.equals(role)) {
                user.setRole(normalized);
                appUserRepository.save(user);
                log.info("AppUser role migrated: email={} {} -> {}", user.getEmail(), role, normalized);
            }
        }
    }
}
