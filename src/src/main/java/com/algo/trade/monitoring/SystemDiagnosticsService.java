    ...
    private DiagnosticSnapshot buildSnapshot() {
        ...
        for (IndexType idx : enabledIndices) {
            ...
            try {
                ...
                var underlying = UnderlyingSymbol.valueOf(idx.name());
        ...
        for (IndexType idx : enabledIndices) {
            ...
            var candles1m = liveCandleBuilder.getHistory(spotToken, Timeframe.ONE_MINUTE);
            var candles5m = liveCandleBuilder.getHistory(spotToken, Timeframe.FIVE_MINUTE);
            var candles15m = liveCandleBuilder.getHistory(spotToken, Timeframe.FIFTEEN_MINUTE);
        ...
        // Market hours check — suppress data alerts outside 9:10–15:35 IST
        java.time.LocalTime marketTime = java.time.LocalTime.now(IST);
        boolean duringMarketHours = !marketTime.isBefore(java.time.LocalTime.of(9, 10))
                && !marketTime.isAfter(java.time.LocalTime.of(15, 35));

        // System alerts (always relevant)
        ...
        if (duringMarketHours && wsConnected && tickAgeSec > 30) alerts.add("🟡 Last tick " + tickAgeSec + "s ago — data may be stale");
        if (duringMarketHours && wsConnected && tickAgeSec > 120) alerts.add("🔴 ZOMBIE WebSocket — connected but no ticks for " + tickAgeSec + "s");
        if (!brokerAuthenticated) alerts.add("🔴 Broker not authenticated — orders will fail");
        if (duringMarketHours && scanGapSeconds > 300) alerts.add("🔴 No scan in " + scanGapSeconds + "s — scanner stuck");
        ...
        // Data alerts (only during market hours — pre-market data gaps are expected)
        if (duringMarketHours && vix <= 0) alerts.add("🔴 VIX feed unavailable — entries blocked");
        ...
        if (duringMarketHours && cachedPcr <= 0) alerts.add("🟡 PCR data unavailable");
        // Order alerts
