...
public class TelegramAlertService {
    ...
    /**
     * Per-user fan-out: when an alert is generated while running in a SECONDARY user's
     * context (copied entries, their exits, their rejections), a copy is sent to that
     * user's own Telegram chat / webhook. The global chat continues to receive everything.
     */
    @org.springframework.beans.factory.annotation.Autowired(required = false)
    private com.algo.trade.multiuser.UserNotificationService userNotificationService;
    ...
    private void send(String text) {
        // Fan out a copy to the current user's personal channel (no-op for the
        // primary/default user or when no user context is set). Done BEFORE the
        // global-config checks so per-user alerts work even if the global chat
        // is unconfigured, and before dedup so each user gets their own copy.
        fanOutToCurrentUser(text);
    ...
    private void fanOutToCurrentUser(String text) {
        if (userNotificationService == null) return;
        if (!com.algo.trade.multiuser.UserContext.isSet()) return;
        Long uid = com.algo.trade.multiuser.UserContext.getUserId();
        if (uid == null || uid.equals(com.algo.trade.multiuser.UserContext.DEFAULT_USER_ID)) return;
        try {
            // Own-channel only: skips silently if the user hasn't linked Telegram /
            // configured a webhook — avoids duplicating into the global chat.
            userNotificationService.sendToUserOwnChannelOnly(uid, text);
        } catch (Exception e) {
            log.debug("Per-user alert fan-out failed for userId={}: {}", uid, e.getMessage());
        }
    }
