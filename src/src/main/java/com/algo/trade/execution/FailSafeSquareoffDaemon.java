            ...
            for (var order : pendingOrders) {
                try {
                    if (order.getBrokerOrderId() != null && !order.getBrokerOrderId().isBlank()) {
                        // Multi-user: cancel against the ORDER OWNER's broker session, not the
                        // scheduler thread's (unset) context — which would otherwise resolve to the
                        // default user's Kite token and fail to cancel another user's pending order,
                        // leaving them with overnight exposure. Mirrors OrderFillWatchdog's pattern.
                        Long ownerId = order.getUserId();
                        if (ownerId != null) {
                            com.algo.trade.multiuser.UserContext.runAs(ownerId,
                        ...
                        log.warn("[FailSafe] Cancelled pending order: userId={} clientOrderId={} brokerOrderId={}",
                                ownerId, order.getClientOrderId(), order.getBrokerOrderId());
                    }
                } catch (Exception e) {
