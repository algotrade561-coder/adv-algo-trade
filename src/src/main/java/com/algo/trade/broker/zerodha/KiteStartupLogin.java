...
public class KiteStartupLogin implements ApplicationRunner, Ordered {
    ...
    public void onKiteLoginSuccess(KiteLoginSuccessEvent event) {
        // The WebSocket connect is idempotent and must run on EVERY login success — re-login,
        // primary-account swap, or token refresh — so the shared market-data feed reconnects
        // against the new credentials. It is intentionally NOT gated by the one-shot
        // scannerStarted flag: previously any login event after the first start (e.g. the
        // KiteLoginSuccessEvent that PrimaryAccountSwapListener republishes on a primary swap)
        // returned early here and left the WebSocket down, so the dashboard/diagnostics/my-trading
        // surfaces all showed "not connected".
        log.info("KiteLoginSuccessEvent received: userId={} — ensuring WebSocket feed + scanner", event.userId());
        connectWebSocket();                    // idempotent — no-op if already connected
        startScannerAfterLoginIfConfigured();  // scanner start remains one-shot (internally guarded)
    }
    ...
    private void connectWebSocket() {
        // Idempotent: callable on every login-success event without risking a duplicate socket.
        if (webSocketClient.isConnected()) {
            log.info("WebSocket already connected — skipping reconnect");
            return;
        }
        try {
