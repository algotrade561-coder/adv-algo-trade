...
public enum StrategyType {
    ...
    MEAN_REVERSION("Mean Reversion",
            "Midday session (11:00-13:30). Buys when price deviates 0.25%+ from VWAP and OI confirms institutions are fading. Tight SL/target, high win rate.", false),

    // ── Spread Strategies (involve selling legs — DISABLED by default) ─────────
    BULL_CALL_SPREAD("Bull Call Spread",
            "Buy ATM CE + Sell OTM CE. Entry on EMA9 > EMA21 (bullish). All legs placed with safety guarantees.", false),
    BEAR_PUT_SPREAD("Bear Put Spread",
            "Buy ATM PE + Sell OTM PE. Entry on EMA9 < EMA21 (bearish). All legs placed with safety guarantees.", false),
    ...
    BUTTERFLY("Butterfly",
            "Buy wings + sell middle (3 legs). Low cost, high reward if market stays near ATM strike.", false),
    CALENDAR_SPREAD("Calendar Spread",
            "Sell near expiry ATM + buy far expiry ATM. Theta decay play — near leg decays faster.", false),
    DIAGONAL_SPREAD("Diagonal Spread",
            "Buy far expiry ATM + sell near expiry OTM. Directional bias + theta income.", false),
    ...
    SYNTHETIC_FUTURES("Synthetic Futures",
            "Buy ATM CE + Sell ATM PE (bullish) or inverse (bearish). Replicates futures P&L with options. Entry on strong EMA divergence.", false),
