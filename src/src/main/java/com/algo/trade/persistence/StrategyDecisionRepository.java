...
public interface StrategyDecisionRepository extends JpaRepository<StrategyDecisionEntity, Long> {
    ...
    /** Per-user signals: find recent signals for a specific user. */
    List<StrategyDecisionEntity> findTop20ByUserIdOrderByTimestampDesc(Long userId);

    /** Per-user: latest signal for a user. */
    Optional<StrategyDecisionEntity> findTopByUserIdOrderByTimestampDesc(Long userId);
