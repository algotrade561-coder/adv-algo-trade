...
public class OptionChainCollector {
    ...
    /** Latest snapshot per underlying for in-memory access by PCRMaxPainTracker / OIHeatmapController. */
    private final java.util.concurrent.ConcurrentHashMap<UnderlyingSymbol, OptionChainSnapshot> latestSnapshots =
    ...
    /**
     * Returns the latest in-memory option chain snapshot for a given underlying.
     * Used by PCRMaxPainTracker and OIHeatmapController.
     */
    public OptionChainSnapshot collectForDisplay(UnderlyingSymbol underlying) {
        return latestSnapshots.get(underlying);
    }
