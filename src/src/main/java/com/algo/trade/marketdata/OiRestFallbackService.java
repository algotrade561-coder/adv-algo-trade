...
public class OiRestFallbackService {
    ...
    private static final long DEFAULT_WS_STALE_MS = 20_000L;
    private static final long DEFAULT_INSTRUMENT_STALE_MS = 180_000L;
    private static final long DEFAULT_MIN_INTERVAL_MS = 20_000L;
    ...
    @Value("${oi-rest-fallback.ws-stale-ms:20000}")
    private long wsStaleMs;

    @Value("${oi-rest-fallback.instrument-stale-ms:180000}")
    private long instrumentStaleMs;

    @Value("${oi-rest-fallback.min-interval-ms:20000}")
    ...
    private void runCheckAndFallback() {
        ...
        if (System.currentTimeMillis() - lastRestCallMs < minIntervalMs) return;
        ...
        if (!restFallbackActive) {
            String reason = wsStale
                    ? "WS connection stale — no tick for >" + wsStaleMs / 1000 + "s"
                    : stale.size() + " ATM strike(s) have no OI sample for >"
                      + instrumentStaleMs / 60_000 + " min (WS alive but quiet OI)";
    ...
    private boolean isWsConnectionStale() {
        ...
        return Duration.between(lastTick, Instant.now()).toMillis() > wsStaleMs;
    }
    ...
    private boolean isOiSampleStale(OptionInstrument opt, long nowMs) {
        ...
        return (nowMs - sampleMs) > instrumentStaleMs;
    }
    ...
    private static long oiSampleAgeMs(Optional<OptionInstrument> opt, long nowMs) {
        ...
        if (sampleMs <= 0) return DEFAULT_INSTRUMENT_STALE_MS + 1;
