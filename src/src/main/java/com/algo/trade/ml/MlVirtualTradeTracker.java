...
public class MlVirtualTradeTracker {
    ...
    private Map<String, Object> tradeToMap(VirtualTrade vt) {
        ...
        BigDecimal priceDiff = (vt.exitPrice != null && vt.exitPrice.signum() > 0 ? vt.exitPrice : vt.currentPrice)
                .subtract(vt.entryPrice);
        m.put("pnlAmount", priceDiff.multiply(BigDecimal.valueOf(qty)).setScale(2, java.math.RoundingMode.HALF_UP));
        m.put("quantity", qty);
        return m;
    }
