...
/**
 * Phase 3 adapter for {@link StrategyType#SCALPING}. Routed through
 * {@link TuningCaptureBridge} after legacy entry-signals recording.
 */
@Component
public class ScalpingCaptureAdapter implements TuningPipelineCapture {
    ...
    public List<BucketDimension> bucketDimensions() {
        return List.of(
                new BucketDimension("scalpCrossType", "scalpCrossType", null, BandStyle.CATEGORICAL),
                new BucketDimension("scalpConfirmCount", "scalpConfirmCount",
    ...
    static Map<String, Object> evaluationAttributes(
        ...
        if (ctx.scalpCrossType() != null) {
            attrs.put("scalpCrossType", ctx.scalpCrossType());
        }
        if (ctx.scalpConfirmCount() != null) {
            attrs.put("scalpConfirmCount", ctx.scalpConfirmCount());
        }
        putIfPresent(attrs, "scalpEma9", ctx.scalpEma9());
        putIfPresent(attrs, "scalpEma21", ctx.scalpEma21());
        putIfPresent(attrs, "emaGapPct", emaGapPct(ctx));
    ...
    static Double emaGapPct(SignalRecordContext ctx) {
        if (ctx.scalpEma9() == null || ctx.scalpEma21() == null || ctx.scalpEma21() == 0.0d) {
            return null;
        }
        return Math.abs(ctx.scalpEma9() - ctx.scalpEma21()) / ctx.scalpEma21() * 100.0d;
    }
