...
public class SnapshotFileWriter {
    ...
    public Optional<Path> write(ChainSnapshot snapshot) {
        ...
        try {
            ...
            // Atomic write: serialize to a temp file first, fully flush/close it, then
            // atomically move it into place. A crash or interruption mid-write leaves only
            // the discardable .tmp file — the published .json.gz is never truncated.
            // (Root-caused from ~5.6% of historical captures being <1KB partial files.)
            Path tmpPath = dateDir.resolve(fileName + ".tmp");
            ...
            try {
                ...
                // Filesystem may not support ATOMIC_MOVE (e.g. some network shares); fall
                // back to a plain replace, which is still safer than streaming to the target.
