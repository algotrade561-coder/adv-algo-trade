...
public class OptionChainSnapshotScheduler {
    ...
    /**
     * Per-strike CUMULATIVE volume from the previous snapshot cycle, keyed by
     * "INDEX|strike|CE_or_PE". Used to derive a per-interval traded-volume delta in
     * buildStrikeData(), because the raw broker volume field is cumulative for the day
     * and momentum/absorption logic needs the volume traded *in this interval*.
     *
     * Cleared on each new trading day alongside the OI/LTP caches.
     */
    private final Map<String, Long> prevSnapshotVolumeCache = new ConcurrentHashMap<>();
    ...
    public void captureSnapshots() {
        ...
        if (lastCaptureDay != null && !today.equals(lastCaptureDay)) {
            ...
            prevSnapshotVolumeCache.clear();
            log.info("[ChainSnapshot] New trading day {} — cleared cross-day OI/LTP/volume snapshot caches", today);
        }
    ...
    private ChainSnapshot.StrikeData buildStrikeData(int strike, OptionInstrument ce, OptionInstrument pe) {
        ...
        // A delta is VALID only when a real prior baseline existed AND both readings are
        // positive. When invalid, we emit 0 but flag it so tuning can ignore the value
        // instead of treating it as a genuine "no change".
        boolean ceOiChangeValid = prevCeOi != null && prevCeOi > 0 && currentCeOi > 0;
        boolean peOiChangeValid = prevPeOi != null && prevPeOi > 0 && currentPeOi > 0;
        long ceOiChange = ceOiChangeValid ? currentCeOi - prevCeOi : 0;
        long peOiChange = peOiChangeValid ? currentPeOi - prevPeOi : 0;

        // ── Per-interval traded volume (cumulative diff) ─────────────────────
        long currentCeVol = ce.getVolume();
        long currentPeVol = pe.getVolume();
        Long prevCeVol = prevSnapshotVolumeCache.get(ceKey);
        Long prevPeVol = prevSnapshotVolumeCache.get(peKey);
        // Volume is cumulative for the day, so the interval volume is the diff. Guard against
        // a cumulative reset (current < prev) which would otherwise yield a negative interval.
        long ceVolumeInterval = (prevCeVol != null && currentCeVol >= prevCeVol)
                ? currentCeVol - prevCeVol : 0;
        long peVolumeInterval = (prevPeVol != null && currentPeVol >= prevPeVol)
                ? currentPeVol - prevPeVol : 0;
        ...
        // Volume is cumulative and only ever increases intraday; cache every reading
        // (including 0 at day-start) so the next interval diff has a baseline.
        prevSnapshotVolumeCache.put(ceKey, currentCeVol);
        prevSnapshotVolumeCache.put(peKey, currentPeVol);
        ...
        return new ChainSnapshot.StrikeData(
                ...
                peHigh5m,
                peLow5m,
                // Capture-quality / interval fields
                ceVolumeInterval,
                peVolumeInterval,
                ceOiChangeValid,
                peOiChangeValid
