...
public class AlgoFlowOrchestrator {
    ...
    @org.springframework.beans.factory.annotation.Autowired(required = false)
    private ExpiryOperatorTrapDetector expiryTrapDetector;
    ...
    public EntryDecision evaluateEntry(UnderlyingSymbol underlying, LocalTime marketTime,
        ...
        // ═══════════════════════════════════════════════════════════════════
        // PHASE 1d: EXPIRY BEHAVIOR TUNER (operator-style auto-tune)
        // ═══════════════════════════════════════════════════════════════════
        if (expiryBehaviorTuner != null) {
            ExpiryBehaviorTuner.ExpiryMode expiryMode = expiryBehaviorTuner.currentMode(underlying);
            if (expiryMode == ExpiryBehaviorTuner.ExpiryMode.CRASH_HALT) {
                failed.add("EXPIRY_TUNER:CRASH_HALT");
                return EntryDecision.blocked(
                        "Market crash halt — " + expiryBehaviorTuner.modeDescription(underlying),
                        passed, failed);
            }
            if (expiryMode == ExpiryBehaviorTuner.ExpiryMode.EXPIRY_REVERSAL
                    && strategyType.isSellingStrategy()
                    && expiryBehaviorTuner.blockNakedSelling(underlying)) {
                failed.add("EXPIRY_TUNER:REVERSAL_NO_NAKED_SELL");
                return EntryDecision.blocked(
                        "Sensex expiry reversal — naked selling blocked, gamma scalps allowed after noon",
                        passed, failed);
            }
            passed.add("EXPIRY_TUNER:" + expiryMode.name());
        }

        // ── PHASE 1e: EXPIRY OPERATOR TRAP DETECTION (post-2PM on expiry) ──
        if (expiryTrapDetector != null) {
            IndexType idx = IndexType.from(underlying);
            if (expiryCalendar.isExpiryDay(idx)) {
                var trap = expiryTrapDetector.assess(idx);
                if (trap.anyTrapActive()) {
                    // Force exit signal if OI collapse detected
                    if (trap.oiUnwindCollapse()) {
                        failed.add("EXPIRY_TRAP:OI_UNWIND_COLLAPSE");
                        return EntryDecision.blocked(
                                "OI unwind collapse at key strike — operator trap, exit positions",
                                passed, failed);
                    }
                    // Block new entries during gamma blast zone (explosive move imminent)
                    if (trap.gammaBlastZone() && !strategyType.name().contains("GAMMA")) {
                        failed.add("EXPIRY_TRAP:GAMMA_BLAST_ZONE");
                        return EntryDecision.blocked(
                                "Gamma blast zone active (2:15-3:00 PM) — only gamma scalps allowed",
                                passed, failed);
                    }
                    // OI divergence: warn but don't block (strategies should reduce size)
                    if (trap.oiDivergenceTrap()) {
                        passed.add("EXPIRY_TRAP:OI_DIVERGENCE(caution)");
                    }
                }
                // Max pain pinning bias: after 2:30 inform strategies of target direction
                if (expiryTrapDetector.isMaxPainBiasActive(idx)) {
                    int mpDir = expiryTrapDetector.getMaxPainDirection(idx);
                    if (mpDir != 0) {
                        passed.add("EXPIRY_PIN:maxPain=" + expiryTrapDetector.getMaxPainTarget(idx)
                                + ",dir=" + (mpDir > 0 ? "BULL" : "BEAR"));
                    }
                }
            }
        }
        ...
        {
            LocalTime underlyingCutoff = underlyingConfigService.getEntryCutoffTime(underlying);
    ...
    private SessionWindow classifySession(LocalTime time, UnderlyingSymbol underlying) {
        ...
        // Sensex expiry reversal override: treat post-gamma-start as GAMMA_SCALPING window
        // (time comes from trading.expiry-tuner.reversal-gamma-start, default 12:00)
        if (expiryBehaviorTuner != null && idx == IndexType.SENSEX
                && expiryBehaviorTuner.currentMode(underlying) == ExpiryBehaviorTuner.ExpiryMode.EXPIRY_REVERSAL
                && time.isAfter(expiryBehaviorTuner.gammaScalpStartTime())) {
