...
public class GapAndGoStrategy implements TimeBoundedStrategy {
    ...
    @Override
    public LocalTime entryStartTime() { return ENTRY_START; }

    @Override
    public LocalTime entryCutoffTime() { return ENTRY_CUTOFF; }
    ...
    private double fetchPreviousDayClose(IndexType idx) {
        ...
        try {
            String spotKey = tradingProperties.symbols().spotHistoricalKeys().get(
                    UnderlyingSymbol.valueOf(idx.name()));
            ...
            var request = new HistoricalDataRequest(
                    spotKey, from, to, Timeframe.FIVE_MINUTE, false);
